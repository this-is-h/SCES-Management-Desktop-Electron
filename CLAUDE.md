...
- CI 跨仓拉取私有依赖 `@sces/shared` 与推送 `updates/latest.json` 依赖 GitHub 仓库 secret `SCES_CI_TOKEN`（与本地 omp:github-pat 同一枚 PAT）。
- 本仓无运行时 `.env`（local-vault 密钥 per-install 生成、落本地 storage，与仓库密钥无关）；`.gitignore` 已补 `.env` 规则防未来误提交。

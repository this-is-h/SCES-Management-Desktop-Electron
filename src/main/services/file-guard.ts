 ...
 *  1. 扩展名白名单：只允许本通道合法的文件类型（如 .dysl/.dysc/.dyf/.dxy），
 *     堵住"任意路径读取 + 错误文案差异构成的存在性/类型探测 oracle"；
   *  2. 大小上限：拒绝超大文件，避免同步 readFileSync 阻塞主进程。
 * 注：应用数据文件（.dyf 学生申请 / .dxy 数据交换）放宽至 1GB（见 MAX_APPLICATION_FILE_BYTES），
 * 真实材料（压缩图片证据）可达数百 MB；license/证书文件保持 50MB 收紧下限。
 */
import { statSync } from 'fs'
import { extname } from 'path'

/** 默认单文件读取上限：50MB（正常 .dyf/.dxy 含证据 base64 也远低于此）。 */
export const DEFAULT_MAX_FILE_BYTES = 50 * 1024 * 1024

/**
 * 应用数据文件读取上限：1GB。
 * .dyf（单个学生材料）与 .dxy（整班/整年级聚合）的证据为压缩图片 base64，
